...
def get_facenet_embeddings(args):
    ...
    path_list, issame_list = get_paths_with_pairs(args.facenet_dataset_dir, pairs)
    nrof_classes = len(dataset)

    # if args.validation_set_split_ratio > 0.0:
    #     dataset, val_set = facenet.split_dataset(dataset, args.validation_set_split_ratio, args.min_nrof_val_images_per_class, 'SPLIT_IMAGES')
    # else:
        ...
        with tf.Session() as sess:
            ...
            while True:
                try:
                    ...
                    # images_flip, labels_flip = sess.run(tf_dataset_next_element_flip)
...
def custom_insightface_evaluation(args):
    ...
    pairs = read_pairs(args.insightface_pair)
    path_list, issame_list = get_paths_with_pairs(args.insightface_dataset_dir, pairs)
    nrof_classes = len(dataset)
...
if __name__ == '__main__':
    ...
    custom_facenet_evaluation(args)
    # exit(0)
    print(f"{'*@*'}"*50)
    print(f"{'*@*'}"*50)
    print(f"{'*@*'}"*50)
    # get_insightface_embeddings(args)
    custom_insightface_evaluation(args)
